...
SingleProxy = FrontendProxy | str
ProxyList = Iterable[SingleProxy]
Proxies = SingleProxy | ProxyList
...
class Schedule:
    ...
    def __init__(self, module: ir.ModuleOp, context: ir.Context):
        ...
        self.valid_handle_cache: dict[int, ir.Value] = {}
        ...
        self.epoch = 0
        self._dirty = False
        self._active_proxy: FrontendProxy | None = None
    ...
    def _init_transform(self):
        ...
        self.builder.set_insertion_point_to_end(seq.entry_block)
        tran_d.YieldOp.create(self.builder, [])

        self.builder.set_insertion_point_to_start(seq.entry_block)
        attr = ir.DictionaryAttr.get(
            self.context,
            {utils.IDENTIFIER_ATTR_NAME: self.builder.get_str_attr("allo_payload")},
        )
        self.payload_root = tran_d.MatchOp.create(
            self.builder, seq.get_arg_at(0), op_ty, ["builtin.module"], attr
        )
        self.valid_handle_cache.clear()
        self.valid_handle_cache[self._proxy_key(self.payload_tree)] = self.payload_root
        self._dirty = False
    ...
    def _resolve_proxies(
        ...
        *,
        ...
        if require_value:
            # remove operation proxies
            for proxy in resolved:
                if isinstance(proxy, OperationProxy):
                    resolved.remove(proxy)
    ...
    def _provide_proxies(self, proxies: list[FrontendProxy], handles: list[ir.Value]):
        ...
        for proxy, handle in zip(proxies, handles):
            ...
            self._register_live_proxy(proxy)
            self.valid_handle_cache[key] = handle
    ...
    def _insert_proxies(
        ...
        for proxy in proxies:
            ...
            self.by_parent.setdefault(proxy.hierarchy_name, [])
            if hasattr(proxy, "set_parent"):
                proxy.set_parent(parent)
    ...
    def _invalidate_proxies(
        ...
        *,
        ...
        for proxy in proxies:
            proxy.state = state
            self.valid_handle_cache.pop(self._proxy_key(proxy), None)
        ...
        *,
        ...
        for proxy in proxies:
            ...
            if isinstance(proxy, OperationProxy):
                ...
                if hasattr(proxy, "set_parent"):
                    proxy.set_parent(None)
            else:
                self._parent_path.pop(key, None)

            self.valid_handle_cache.pop(key, None)
        ...
        if (
            ...
            and self._proxy_key(self._active_proxy) not in self._live_proxy_keys
        ):
            self._select_fallback_active()
    ...
    def _descendant_invalidate(
        ...
        *,
        ...
        for op_proxy in self._get_descendants(proxy, include_root=include_root):
            self.valid_handle_cache.pop(self._proxy_key(op_proxy), None)
            for value in op_proxy.values:
                self.valid_handle_cache.pop(self._proxy_key(value), None)
    ...
    def _materialize_proxies(
        ...
        for proxy in proxies:
            cache = self.valid_handle_cache.get(self._proxy_key(proxy))
            ...
            if proxy is self.payload_tree:
                ...
                attrs = ir.DictionaryAttr.get(
                    self.context,
                    {
                        utils.IDENTIFIER_ATTR_NAME: self.builder.get_str_attr(
                            proxy.hierarchy_name
                        )
                    },
                )
                handle = tran_d.MatchOp.create(
                ...
                owner_match = tran_d.MatchOp.create(
                    self.builder,
                    self.payload_root,
                    any_op_ty,
    ...
    def from_module(cls, module: ir.ModuleOp) -> "Schedule":
        return cls(module, module.context)
    ...
    def _format_map(self) -> str:
        ...
        for key in self.valid_handle_cache:
            path = "<unknown>"
    ...
    def dump_state(
        ...
        *,
        ...
        lines = [
            ...
            f"epoch={self.epoch}",
            f"dirty={self._dirty}",
            f"active={self._active_proxy.identifier if self._active_proxy is not None else '<none>'}",
            f"live_proxies={len(self._live_proxy_keys)}",
            f"cached_handles={len(self.valid_handle_cache)}",
            f"effects={len(self.effect_log)}",
        ]
        if include_tree:
            lines.append("--- tree ---")
    ...
    def refresh(self):
        ...
        failed, err_msg = tran_d.apply_transforms(
            self.payload.operation, self.transform_seq.operation, self.transform_mod
        )
        if failed:
    ...
    def cse(self, targets: Proxies | None = None):
        ...
        for handle in handles:
            tran_d.ApplyCSEOp.create(self.builder, handle)
        self._mark_dirty()
        if resolved:
    ...
    def dce(self, targets: Proxies | None = None):
        ...
        self._refresh_builder_loc_from_callsite()
        for handle in handles:
            tran_d.ApplyDCEOp.create(self.builder, handle)
        self._mark_dirty()
        if resolved:
            self._set_active_proxy(resolved[-1])
        self._record_effect(
    ...
    def licm(self, targets: Proxies | None = None):
        ...
        self._refresh_builder_loc_from_callsite()
        for handle in handles:
            tran_d.ApplyLICMOp.create(self.builder, handle)
        self._mark_dirty()
        if resolved:
            self._set_active_proxy(resolved[-1])
        self._record_effect(
    ...
    def apply_patterns(
        ...
        self._refresh_builder_loc_from_callsite()
        for handle in handles:
            entry = tran_d.ApplyPatternsOp.create(self.builder, handle)
            ip = self.builder.save_insertion_point()
            self.builder.set_insertion_point_to_end(entry.body)
            for op_cls in pattern_ops:
                op_cls.create(self.builder)
    ...
    def polyhedral(self, targets: Proxies | None = None):
        ...
        for proxy, handle in zip(resolved, handles):
            outputs.append(tran_d.RaiseToAffineOp.create(self.builder, handle))
    ...
    def unroll(
        ...
        for proxy, handle in zip(resolved, handles):
            if tag_only:
                tran_d.TagUnrollOp.create(self.builder, handle, factor)
            else:
                tran_d.LoopUnrollOp.create(self.builder, handle, factor)
    ...
    def flatten(self, targets: Proxies | None = None):
        ...
        self._refresh_builder_loc_from_callsite()
        merged = tran_d.MergeHandlesOp.create(self.builder, handles, deduplicate=True)
        flattened = tran_d.LoopFlattenOp.create(self.builder, merged)
    ...
    def tile(self, targets: Proxies | None = None, factors: int | list[int] = 1):
        ...
        self._refresh_builder_loc_from_callsite()
        merged = tran_d.MergeHandlesOp.create(self.builder, handles, deduplicate=True)
        tiled = tran_d.LoopTileOp.create(self.builder, merged, factors)
        tile_group = tiled.get_result_at(0)
        point_group = tiled.get_result_at(1)
        tile_split = tran_d.SplitHandleOp.create(
            self.builder, tile_group, len(sorted_resolved)
        )
        point_split = tran_d.SplitHandleOp.create(
    ...
    def pipeline(self, targets: Proxies | None = None, ii: int = 1):
        ...
        self._refresh_builder_loc_from_callsite()
        for handle in handles:
            tran_d.TagPipelineOp.create(self.builder, handle, ii)
    ...
    def split(self, targets: Proxies | None = None, factor: int = 1):
        ...
        self._refresh_builder_loc_from_callsite()
        for proxy, handle in zip(resolved, handles):
            split_op = tran_d.LoopSplitOp.create(self.builder, handle, factor)
    ...
    def outline(self, target: SingleProxy | None = None, *, func_name: str):
        ...
        self._refresh_builder_loc_from_callsite()
        outlined = tran_d.OutlineOp.create(self.builder, handle, func_name)
    ...
    def reorder(self, targets: Proxies, order: list[int]):
        ...
        self._refresh_builder_loc_from_callsite()
        merged = tran_d.MergeHandlesOp.create(self.builder, handles, deduplicate=False)
        tran_d.LoopReorderOp.create(self.builder, merged, order)
        split_back = tran_d.SplitHandleOp.create(
            self.builder, merged, len(affine_loops)
        )
    ...
    def compute_at(self, target: SingleProxy, axis: SingleProxy):
        ...
        target_handle, axis_handle = self._materialize_proxies(
            [target_proxy, axis_proxy]
        )
        transform_id = self._next_transform_id("compute_at")

        self._refresh_builder_loc_from_callsite()
        tran_d.ComputeAtOp.create(self.builder, target_handle, axis_handle)
    ...
    def reuse_at(self, target: str | ValueProxy, axis: SingleProxy) -> ValueProxy:
        ...
        value_proxy = resolved_values[0]
        if value_proxy.kind_str != "buffer":
            raise ValueError(
                f"Value '{value_proxy.hierarchy_name}' is not a buffer and cannot be reused"
            )
        ...
        self._refresh_builder_loc_from_callsite()
        reuse_op = tran_d.ReuseAtOp.create(self.builder, target_handle, axis_handle)
        alloc_handle = reuse_op.get_result_at(0)

        value_identifier = f"{value_proxy.identifier}::reuse@{axis_proxy.identifier}"
        alloc_identifier = value_identifier + "::alloc"
        alloc_hierarchy = f"{root_parent.hierarchy_name}.Oreuse{self._effect_counter}"
        ...
        value_handle = tran_d.MatchValueOp.create(self.builder, alloc_handle, 0, 2)

        self._provide_proxies(
            [alloc_proxy, value_proxy],
            [alloc_handle, value_handle],
        )
        ...
        self._record_effect(
            transform_id,
            "reuse_at",
            target=value_proxy.hierarchy_name,
            axis=axis_proxy.hierarchy_name,
    ...
    def partition(
        ...
        *,
        dim: int = 0,
        kind: tran_d.PartitionKind = tran_d.PartitionKind.Complete,
        factor: int = 0,
    ):
        ...
        if kind == tran_d.PartitionKind.Complete:
            if factor != 0:
        ...
        part = tran_d.PartitionAttr.get(self.context, [(dim, kind.value, factor)])

        self._refresh_builder_loc_from_callsite()
        for h in handles:
            tran_d.PartitionOp.create(self.builder, h, part)
