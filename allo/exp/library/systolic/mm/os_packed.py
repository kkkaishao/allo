...
def make_packed_output_stationary_gemm(
    ...
    """Build + schedule a DSP-packed output-stationary GEMM; return ``(top, top_s)``.

    Low-bitwidth integer only. ``A`` is pre-transposed ``[K,M]``. A packed PE owns
    ``P`` adjacent output columns that share the eastward-flowing activation: their
    ``P`` ``B`` values are packed into one operand, one wide multiply yields all
    ``P`` products, and a signed borrow-chain unpacks them into ``P`` per-column
    accumulators. The array is ``Mt x (Nt/P)``. ``G`` is the product bit gap;
    requires ``Nt % P == 0``."""
    assert (
    ...
    assert M % Mt == 0 and N % Nt == 0 and Nt % P == 0, "tiling must be even"
