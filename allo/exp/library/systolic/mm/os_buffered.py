...
def make_buffered_output_stationary_gemm(
    ...
    """Build + schedule the N-blocked buffered OS systolic GEMM; return ``(top, top_s)``.

    ``At`` is passed pre-transposed (shape ``[K,M]``). ``Nc`` is the column-block
    width (number of ``C`` columns processed per outer block); it bounds the B
    buffer to ``[K,Nc]`` and is the buffer-size vs DRAM-traffic knob (see the
    ``top`` docstring). Must satisfy ``N % Nc == 0`` and ``Nc % Nt == 0``; defaults
    to ``N`` (buffer all of B, A read once). ``depth`` is the inter-process FIFO
    depth; bump it if co-simulation reports a stall."""
    assert (
        isinstance(Tin, DType) and isinstance(Tacc, DType) and isinstance(Tout, DType)
    ), "Tin/Tacc/Tout must be Allo DType"
    assert M % Mt == 0 and N % Nt == 0, "array must tile the matrix evenly"
    Nc = N if Nc is None else Nc
    assert N % Nc == 0 and Nc % Nt == 0, "Nc must divide N and be a multiple of Nt"
