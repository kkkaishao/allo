...
def twiddles8(a_x: "f32[8]", a_y: "f32[8]", i: i32, n: i32):
    ...
    for j in range(1, 8):
        ...
        phi_x: f32 = allo_math.cos(phi)
        phi_y: f32 = allo_math.sin(phi)
        tmp_1: f32 = a_x[j]
...
@kernel
def FF2(a0_x: f32, a0_y: f32, a1_x: f32, a1_y: f32) -> "f32[4]":
    d0: "f32[4]" = 0.0
...
@kernel
def loadx8(a_x: "f32[8]", x: "f32[8 * 8 * 9]", offset: i32, sx: i32):
    a_x[0] = x[0 * sx + offset]
    a_x[1] = x[1 * sx + offset]
    a_x[2] = x[2 * sx + offset]
    a_x[3] = x[3 * sx + offset]
...
@kernel
def loady8(a_y: "f32[8]", x: "f32[8 * 8 * 9]", offset: i32, sx: i32):
    a_y[0] = x[0 * sx + offset]
    a_y[1] = x[1 * sx + offset]
    a_y[2] = x[2 * sx + offset]
    a_y[3] = x[3 * sx + offset]
...
@kernel
def fft1D_512(work_x: "f32[512]", work_y: "f32[512]"):
    stride: i32 = 64
    counter: i32 = 0
    reversed: "i32[8]" = [0, 4, 2, 6, 1, 5, 3, 7]
