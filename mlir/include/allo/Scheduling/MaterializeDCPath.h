...
/// Reify the solved schedule of every scheduled func in \p module -- carried
/// transiently as `allo.sched.*` attributes -- into `allo.dcp.*` ops
/// (dcp.operator declarations, dcp.pipeline/sequential regions, dcp.compute/
/// load/store), then strip the carrier. Operator latencies come from \p lib.
