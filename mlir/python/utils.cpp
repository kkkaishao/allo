...
void ValueProxy::dump(llvm::raw_string_ostream &os) const {
  ...
  os << identifier << " ";
  dumpState(os);
  os << " alias=" << hierarchy_name << "\n";
}

void OperationProxy::dumpOpType(llvm::raw_string_ostream &os) const {
  std::string ss;
  // control flows
  StringRef kind_ref = kind_str;
  if (kind_ref == "func.func") {
    ss += "function ";
  } else if (kind_ref.contains("for") || kind_ref.contains("parallel")) {
    ss += "loop ";
  } else if (kind_ref.contains("if")) {
    ss += "branch ";
  }
  // effects
  if (kind_ref.contains("alloc") || kind_ref.contains("empty"))
    ss += "alloc ";
  if (kind_ref.contains("dealloc"))
    ss += "free ";
  if (kind_ref.contains("load"))
    ss += "read ";
  if (kind_ref.contains("store"))
    ss += "write ";
...
static void collectBufferProxies(OperationProxy &opProxy) {
  for (auto v : opProxy.inst->getResults()) {
    if (isa<BaseMemRefType, TensorType>(v.getType())) {
      ...
      valueProxy->inst = v;
      valueProxy->kind_str = "buffer";
      valueProxy->number = v.getResultNumber();
      valueProxy->hierarchy_name =
          opProxy.hierarchy_name + ":res" + std::to_string(v.getResultNumber());
      if (auto idAttr =
        ...
        valueProxy->identifier = valueProxy->hierarchy_name;
  ...
  for (auto arg : funcOp.getArguments()) {
    if (isa<BaseMemRefType, TensorType>(arg.getType())) {
      ...
      valueProxy->hierarchy_name =
          opProxy.hierarchy_name + ":arg" + std::to_string(arg.getArgNumber());
...
buildOperationProxy(OperationTreeState &state, Operation *op,
  ...
  proxy->parent = parent;
  proxy->inst = op;
  proxy->kind_str = op->getName().getStringRef().str();
  ...
  if (auto idAttr = op->getAttrOfType<StringAttr>(allo::OpIdentifier))
    ...
    proxy->identifier = proxy->hierarchy_name;

  // we only care about buffer values for now.
  collectBufferProxies(*proxy);

  if (isCtrlFlow)
    state.ctrlStack.push_back(proxy->hierarchy_name);
  // save state
...
void OperationProxy::updateChildrenRec() {
  OperationTreeState state;
  state.ctrlStack.push_back(hierarchy_name);
  for (auto &nested : inst->getRegions()) {
    for (auto &block : nested.getBlocks()) {
      for (auto &nestedOp : block.getOperations()) {
        if (nestedOp.hasTrait<OpTrait::IsTerminator>())
          continue; // skip terminator
        children.push_back(buildOperationProxy(state, &nestedOp, this));
      }
    }
  }
}
...
static std::unique_ptr<OperationProxy> buildOperationTree(ModuleOp mod) {
  auto proxy = std::make_unique<OperationProxy>();
  proxy->inst = mod;
  proxy->kind_str = mod.getOperationName();
  proxy->identifier = "allo_payload";
  proxy->hierarchy_name = "allo_payload";
  OperationTreeState state;
...
static void completeHierarchyName(OperationProxy &proxy) {
  if (!isa<SymbolOpInterface>(proxy.inst)) {
    proxy.inst->setAttr(
        allo::OpIdentifier,
        StringAttr::get(proxy.inst->getContext(), proxy.hierarchy_name));
  }
...
static std::string IDENTIFIER_ATTR_NAME = allo::OpIdentifier.data();
...
void init_utils(nb::module_ &m) {
  ...
  nb::class_<FrontendProxy>(m, "FrontendProxy")
      .def_prop_rw(
          "kind_str", [](FrontendProxy &self) { return self.kind_str; },
          [](FrontendProxy &self, std::string_view kind) {
            self.kind_str = kind;
          })
      .def_prop_rw(
          "hierarchy_name",
          [](FrontendProxy &self) { return self.hierarchy_name; },
  ...
  nb::class_<OperationProxy, FrontendProxy>(m, "OperationProxy")
      ...
      .def(
          ...
          [](OperationProxy &self, unsigned index, std::string_view id,
             std::string_view kind, std::string_view hier_name) {
            auto child = std::make_unique<OperationProxy>();
            child->inst = nullptr;
            child->identifier = id;
            child->hierarchy_name = hier_name;
            child->kind_str = kind;
            child->parent = &self;
      ...
      .def(
          ...
          [](OperationProxy &self, std::string_view id, std::string_view kind,
             std::string_view hier_name, unsigned number = 0) {
            auto value = std::make_unique<ValueProxy>();
            value->owner = &self;
            value->identifier = id;
            value->kind_str = kind;
            value->hierarchy_name = hier_name;
            value->number = number;
  ...
  nb::class_<ValueProxy, FrontendProxy>(m, "ValueProxy")
      .def_prop_ro(
          ...
          nb::rv_policy::reference)
      .def_prop_ro("number", [](ValueProxy &self) { return self.number; });
